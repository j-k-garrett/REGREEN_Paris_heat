...
## Read in data
setwd(data_d)
step_1_results_files<-dir(pattern = "step_1_resuts_575_150m")
step_1_results<-read.csv(step_1_results_files)
```
...
## Boxplot with whiskers at the 1st and 99th percentile. outliers excluded
p<-ggplot(data=step_1_results, aes(x =area_cat, y = NDVI ))+ 
  stat_summary(geom = "boxplot",   fun.data = function(x) setNames(quantile(x, c(0.01, 0.25, 0.5, 0.75, 0.99)), 
...
p
setwd(figure_d)
ggsave("boxplot_ndvi.png", dpi = 600, width = 4.5, height = 3.5, bg = "white")
rm( p)
```
...
plot_grid(p1, p2)
setwd(figure_d)
ggsave("gam_plots_maximum_cool1_dist.png", width = 7.5, height = 4, units ="in", bg = "white")
summary(gam_model)
...
plot_grid(p1, p2)
setwd(figure_d)
ggsave("gam_plots_temp_diff.png", width = 7.5, height = 4, units ="in", bg = "white")
...
setwd(data_d)
writeVector(Parks_grtr_1000, paste0("Parks_grtr_1000_with_preds_",Sys.Date(),".shp"))
...
setwd(figure_d)
ggsave("map_greenspaces_mcd.png", dpi = 600, width = 6, height = 5, bg = "white")
